...
public class GeoRoutingProtocol {
    ...
    public GeoRoutingProtocol(SuperNode sn) {
        this.sn = sn;
    ...
    public Node iterativeFindNodeLocal(Node targetNode, double lat, double lon, GeoDistributedHashTableImpl dht) {
        RoutingStatistic stats = new RoutingStatistic();
        long timeStart = System.nanoTime();

        SortedSet<Node> shortlist = new TreeSet<>(GeoDistributedHashTableImpl.getGeoNodeComparator(lat, lon));
        SortedSet<Node> probed = new TreeSet<>(GeoDistributedHashTableImpl.getGeoNodeComparator(dht.getLat(), dht.getLon()));

        int alpha = dht.getConfiguration().getAlpha();
        int k = dht.getConfiguration().getK();

        SortedSet<Node> oldestNodes = dht.getOldestNodes(targetNode.getId(), lat, lon, 3);
        for (Node n : oldestNodes) {
    ...
    public Node iterativeFindNode(Number160 targetId, double lat, double lon, GeoDistributedHashTableImpl dht) {
...
//        SortedSet<Node> oldestNodes = dht.getOldestNodes(targetId, dht.getLat(), dht.getLon(), 3);
        SortedSet<Node> oldestNodes = dht.getOldestNodes(targetId, lat, lon, 3);
        for (Node n : oldestNodes) {
//            if (n.getId().equals(id)) continue; // don't query the node, if we already know it... (yeah, this method is also used for other purposes than finding the node)
        ...
        while ((!allProbed(shortlist, probed) && probed.size() < k) && tmplocked < 2222) {
            tmplocked++;
            dhtStates.add(dht.getFormattedKBuckets());
            stats.increaseIterationCount();
...
//            }
                ...
                for (Node node : shortlist) {
                    if (!probed.contains(node) && count < alpha) {
                        ...
                        RpcMessages.Header header = RpcMessages.Header.newBuilder()
                            ...
                            .setNetworkId("0")
                            .build();
                        RpcMessages.Ext ext = RpcMessages.Ext.newBuilder()
                            .putDict("x", node.getDict("x"))
                            .putDict("y", node.getDict("y"))
                            .build();
            ...
            for (ValueOrNodesFuture f : futures) {
                try {
                    ...
                    for (RpcMessages.Peer p : r.getPeersList()) {
                        ...
                        node.setPeer(peer);
                        if (p.getExt() != null && p.getExt().getDictMap() != null && p.getExt().containsDict("x") && p.getExt().containsDict("y")) {
                            node.putDict("x", p.getExt().getDictOrDefault("x", "0"));
                            node.putDict("y", p.getExt().getDictOrDefault("y", "0"));
                        }
                        // do not add self to shortlist nor dht
                        if (!node.getId().equals(overlay.getSelf().getId())) {
                            nodes.add(node);
        ...
        if (tmplocked > 1000) {
            log.error("Unable to get node from shortlist, shortlist size: " + shortlist.size() + " probed size: " + probed.size());
        }
    ...
    public byte[] iterativeFindValue(Number160 key, GeoDistributedHashTableImpl dht) {
//        System.out.println("iterative find value goal: " + key);
        DhtStates dhtStates = new DhtStates(statLogger);
        RoutingStatistic stats = new RoutingStatistic();
        long timeStart = System.nanoTime();

        Set<Node> shortlist = new TreeSet<>(GeoDistributedHashTableImpl.getGeoNodeComparator(dht.getLat(), dht.getLon()));
...
//            shortlist.add(n);
//        }
        shortlist.addAll(dht.getOldestNodes(key, dht.getLat(), dht.getLon(), 3));
    ...
    public void iterativeStore(Number160 key, byte[] value, GeoDistributedHashTableImpl dht, int nodestorecount) {
//        System.out.println("iterative store goal: " + key);
        ...
        int alpha = dht.getConfiguration().getAlpha();
        int k = dht.getConfiguration().getK();
        shortlist.addAll(dht.getOldestNodes(key, dht.getLat(), dht.getLon(), 3)); // TODO: <-- difference from findnode, but should be possible to reuse code
...
//                        log.debug("Store key: " + key.toString() + ", success: " + result.equals("1"));
    ...
    public Map<String,byte[]> iterativeFindMap(Number160 key, GeoDistributedHashTableImpl dht) {
//        System.out.println("iterative find map goal: " + key);
    ...
    public void iterativeStoreMapValue(Number160 key, String index, byte[] value, GeoDistributedHashTableImpl dht) {
