...
public class AddValueHandler extends SimpleChannelInboundHandler<RpcMessages.AddValue> {
    private final Logger log = LoggerFactory.getLogger(this.getClass());
    private final SuperNode sn;
    private StatLogger statLogger;

    public AddValueHandler(SuperNode reference) {
        this.sn = reference;
        this.statLogger = reference.getStatLogger();
    }

    @Override
    protected void channelRead0(ChannelHandlerContext ctx, RpcMessages.AddValue msg) throws Exception {
        ...
        if (bs != null && !bs.isEmpty()) {
            ...
            reqo.putValueInMap(key, index, bytes);
            result = "1";
        }
    ...
    private void addToStats(long timestamp, long total, int bytesize) {
        ...
        this.statLogger.addValues("addvaluehandler", list);
    }
}
