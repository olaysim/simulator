...
public class KademliaOverlay implements OverlayNetwork, RequestOverlay {
    ...
    private KademliaOverlay(Builder builder) {
        ...
        DistributedHashTableImpl dht = (DistributedHashTableImpl)distributedHashTable;
    ...
    public void resetDHT() {
        distributedHashTable = DistributedHashTableFactory.createDistributedHashTable(self.getId(), self.getIdStr(), statLogger);
        superNode.reset();
    }

    @Override
    public void saveDhtState() {
        ((DistributedHashTableImpl)distributedHashTable).saveState();
    }

    @Override
    public void reloadDhtState() {
        ((DistributedHashTableImpl)distributedHashTable).reloadState();
    }

    @Override
    public Node findNode(Number160 key) {
        return routingProtocol.iterativeFindNode(key, (DistributedHashTableImpl)distributedHashTable);
    }

    @Override
    public Node findNodeLocal(Node target, double lat, double lon) {
        return routingProtocol.iterativeFindNodeLocal(target, (DistributedHashTableImpl)distributedHashTable);
    }

    @Override
    public void storeValue(Number160 key, byte[] value, int nodestorecount) {
        routingProtocol.iterativeStore(key, value, (DistributedHashTableImpl)distributedHashTable, nodestorecount);
    }

    @Override
    public byte[] findValue(Number160 key) {
        return routingProtocol.iterativeFindValue(key, (DistributedHashTableImpl)distributedHashTable);
    }

    @Override
    public SortedSet<Node> getNodes(Number160 id, Map<String, String> ext) {
        synchronized (lock) {
            return ((DistributedHashTableImpl) distributedHashTable).getClosestNodes(id);
        }
    }
    ...
    public SortedSet<Node> getNodes(Node targetNode) {
        synchronized (lock) {
            return ((DistributedHashTableImpl) distributedHashTable).getClosestNodesLocal(targetNode);
        }
    }

    @Override
    public void storeValueInMap(Number160 key, String index, byte[] value) {
        routingProtocol.iterativeStoreMapValue(key, index, value, (DistributedHashTableImpl)distributedHashTable);
    }

    @Override
    public Map<String, byte[]> findMap(Number160 key) {
        return routingProtocol.iterativeFindMap(key, (DistributedHashTableImpl)distributedHashTable);
    }
    ...
    public boolean updateNode(Node node) {
        synchronized (lock) {
            return ((DistributedHashTableImpl) distributedHashTable).updateNode(node);
        }
    }
...
// Builder pattern
    ...
    public static final class Builder {
        ...
        public Builder routingProtocol(RoutingProtocol val) {
            routingProtocol = val;
            return this;
        }
        ...
        public Builder distributedHashTable(String id, StatLogger statLogger) {
            distributedHashTable = DistributedHashTableFactory.createDistributedHashTable(Number160.createHash(statLogger, id), id, statLogger);
            return this;
        }

        public Builder distributedHashTable(String id, DistributedHashTableConfiguration configuration, StatLogger statLogger) {
            distributedHashTable = DistributedHashTableFactory.createDistributedHashTable(Number160.createHash(statLogger, id), id, configuration, statLogger);
            return this;
        }
        ...
        public KademliaOverlay build() {
            return new KademliaOverlay(this);
        }
    }

    public int getKBucketCount() {
        return ((DistributedHashTableImpl)distributedHashTable).valuesInBuckets();
    }

    public Map<String, List<List<String>>> getFormattedKBuckets() {
        return ((DistributedHashTableImpl)distributedHashTable).getFormattedKBuckets();
    }
